...
// list of courses under a category//

function course_undercat($catid) {
...
$rs = $DB->get_records_sql($qu1);

	return $rs;
}

/* find if a user is enrolled in a course or not   */
function user_yes_course($courseid, $uid){	
global $CFG, $DB;
$yes = 0;
$squery = "SELECT u.id
FROM {course} c
...
where c.id = $courseid AND u.id = $uid ";
$enrolled = $DB->get_record_sql($squery);
if ($enrolled) {
$yes = 1;
} else {
$yes = 0;
...
function enrolledcourse($userid) {
	global $DB,$CFG;
	
	$encourse = $DB->get_record_sql("SELECT courseid FROM {enrol} e
			JOIN {user_enrolments} en ON 
			e.id = en.enrolid 
